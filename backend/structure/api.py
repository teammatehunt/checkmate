...
class UserSerializer(serializers.ModelSerializer):
    socialaccounts = SocialAccountSerializer(
...
class ContainerSpecialization:
    def process_items(self, request, pk=None):
        if request.method == "POST":
            ...
            try:
                items = [
                    getattr(relation, self.relation_model.ITEM)
                    for relation in getattr(
                        self.model.objects.get(pk=pk),
                        f"{self.relation_model.ITEM}_relations",
                    ).prefetch_related(self.relation_model.ITEM)
                ]
...
class PuzzleViewSet(ContainerSpecialization, viewsets.ModelViewSet):
    ...
    queryset = model.objects.all().prefetch_related(
        "round_relations", "meta_relations", "feeder_relations"
    )
...
def process_relation(cls, pk, request):
    ...
    try:
        ...
        if (
            ...
            or not slugs
            or not all(isinstance(slug, str) for slug in slugs)
        ):
            raise exceptions.NotAcceptable(
                "puzzles must be a nonempty list of puzzle slugs"
            )
        if action == "move":
            ...
            if not isinstance(order, int):
                raise exceptions.NotAcceptable(
                    "move operation requires an integer order parameter"
                )
        if data:
            raise exceptions.NotAcceptable(
                f"request has extra fields: {list(data.keys())}"
            )

        # validate inputs slugs
        valid_slugs_query = item_cls.objects.filter(pk__in=slugs).values_list(
            "pk", flat=True
        )
        ...
        existing_slugs_query = existing_relations_query.values_list(
            f"{cls.ITEM}_id", flat=True
        )
        if action == "add":
            ...
            if new_slugs:
                with transaction.atomic():
                    for slug in new_slugs:
                        cls(
                            **{f"{cls.CONTAINER}_id": pk, f"{cls.ITEM}_id": slug}
                        ).save()
                return response.Response(status=status.HTTP_204_NO_CONTENT)
            else:
            ...
            relations_to_remove = [
                ...
                if getattr(relation, f"{cls.ITEM}_id") in slugs
            ]
            ...
            now = datetime.datetime.now()
            with transaction.atomic():
                item_cls.objects.filter(pk__in=existing_slugs_query).update(
                    modified=now, modified_by=request.user
                )
                existing_relations_query.delete()
                cls.objects.bulk_create(new_relations)
                item_cls.objects.filter(pk__in=valid_slugs_query).update(
                    modified=now, modified_by=request.user
                )
                container_cls.objects.filter(pk=pk).update(
                    modified=now, modified_by=request.user
                )
            return response.Response(status=status.HTTP_204_NO_CONTENT)
        elif action == "move":
            ...
            if not 0 <= order < len(relations):
                raise exceptions.NotAcceptable(
                    f"order is out of range: {order} is not in [0, {len(relations)})"
                )
            ...
            if new_order != existing_relation.order:
                with container_cls.get_lock(pk):
                    with transaction.atomic():
                        if new_order < existing_relation.order:
                            relations_set.filter(
                                order__gte=new_order, order__lt=existing_relation.order
                            ).update(order=F("order") + 1)
                        else:
                            relations_set.filter(
                                order__gt=existing_relation.order, order__lte=new_order
                            ).update(order=F("order") - 1)
...
def data_everything():
    ...
    hunt_config = {
        key: getattr(hunt_config, key) for key in HuntConfigSerializer().fields.keys()
    }
    users = cast(
        User.objects.values(
            *(key for key in UserSerializer().fields.keys() if key != "socialaccounts")
        )
    )
    socialaccounts = cast(
        SocialAccount.objects.values(
            "user_id", *SocialAccountSerializer().fields.keys()
        )
    )
...
def discord_voice_move(request):
    socialaccount = SocialAccount.objects.filter(
        provider="discord", user_id=request.user.id
    ).first()
