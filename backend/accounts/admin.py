...
class SocialAccountAdapter(DefaultSocialAccountAdapter):
    ...
    def add_google(self, request, sociallogin):
        ...
        missing_scopes = set(settings.SOCIALACCOUNT_PROVIDERS["google"]["SCOPE"]) - set(
            scopes.split()
        )
        if missing_scopes:
            ...
            raise ImmediateHttpResponse(
                redirect("/accounts/social/login/error/")
            ) from e
        ...
        if not GoogleManager.sync_check_access(owner):
            e = ValueError(
                f"{owner.email} does not have access to the template sheet and the puzzle folder"
            )
            messages.error(request, e)
            raise ImmediateHttpResponse(
                redirect("/accounts/social/login/error/")
